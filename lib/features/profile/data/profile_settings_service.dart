  ...
  Future<AppSetting> _ensureSettingsRow() async {
    ...
    if (rows.isNotEmpty) {
      ...
      if (rows.length > 1) {
        ...
        await (_db.delete(_db.appSettings)..where((t) => t.id.isIn(extraIds))).go();
      }
      return primary;
    }

    final id = await _db.into(_db.appSettings).insert(
          AppSettingsCompanion.insert(
            isDarkMode: const Value(true),
            hasCompletedOnboarding: const Value(false),
          ),
        );
    return (_db.select(_db.appSettings)..where((t) => t.id.equals(id))).getSingle();
  }
