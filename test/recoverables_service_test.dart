...
void main() {
  ...
  test('person-level recovery can settle unbilled card recoverables', () async {
    final now = DateTime.now();
    final billedDate = DateTime(
      now.year,
      now.month,
    ...
    expect(before.groups.single.actionableTotal, closeTo(950, 0.01));
    expect(result.clamped, isTrue);
    expect(result.openBefore, closeTo(950, 0.01));
    expect(result.appliedAmount, closeTo(950, 0.01));
    expect(result.remainingAfter, closeTo(0, 0.01));
    expect(byTitle['Billed card']!.recoverableStatus, 'recovered');
  ...
  test('direct mark recovered accepts unbilled card recoverables', () async {
    ...
    expect(unchanged.recoverableStatus, 'recovered');
    expect(unchanged.recoveredAmount, closeTo(400, 0.01));
  });
